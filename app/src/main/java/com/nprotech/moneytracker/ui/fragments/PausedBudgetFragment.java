...
@AndroidEntryPoint
public class PausedBudgetFragment extends Fragment {
    ...
    private void bindData() {
        try {
            ...
            budgetViewModel.getBudgets(PreferenceManager.INSTANCE.getAccountId(), false, true,
            ...
            budgetAdapter = new RecyclerViewAdapter<>(requireActivity(), new ArrayList<>(), R.layout.item_budget_detail) {
                ...
                public void onPostBindViewHolder(ViewHolder holder, BudgetWithDetails budgetWithDetails) {
                    ...
                    if (spentAmount > budgetAmount) {
                        ...
                        CommonUtils.setDrawable(requireActivity(), tvBudgetStatus, R.drawable.ic_pause, R.dimen.icon_10, R.color.color_pause_dark, Gravity.START);
    ...
    private void showResumeDialog(BudgetEntity budget) {
        ...
        cardHeader.setCardBackgroundColor(requireActivity().getColor(R.color.resume_light));
        headerImage.setImageDrawable(ContextCompat.getDrawable(requireActivity(), R.drawable.ic_resume_outline));
        headerImage.setImageTintList(ColorStateList.valueOf(ContextCompat.getColor(requireActivity(), R.color.resume_dark)));
        tvDelete.setBackgroundTintList(ColorStateList.valueOf(ContextCompat.getColor(requireActivity(), R.color.resume_dark)));
    ...
    private void resumeBudget(BudgetEntity budget) {
        try {
            ...
            if (budgetViewModel.pauseBudget(budget.id, false)) {
                Toast.makeText(requireActivity(), getString(R.string.budget_resumed), Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(requireActivity(), getString(R.string.error_budget_resume), Toast.LENGTH_SHORT).show();
            }
        } catch (Exception e) {
